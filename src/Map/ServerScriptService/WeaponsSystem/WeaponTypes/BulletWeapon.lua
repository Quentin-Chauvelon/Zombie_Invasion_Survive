...
local Utilities, Weapons
if IsServer then
	Utilities = require(ServerScriptService:WaitForChild("Utilities"))
	Weapons = require(ServerScriptService:WaitForChild("Weapons"))
end

-- declare the variable only if it's on the server otherwise the client yields forever because he can't find anything in the server storage
local DamageZombie
if IsServer then
	DamageZombie = ServerStorage:WaitForChild("DamageZombie")
end
...
function BulletWeapon:onEquippedChanged()
	...
	if not IsServer then
		...
		if self.player then
			if self.equipped then
				UpdateAmmoCount:FireClient(self.player, self:getAmmoInWeapon())

				-- set the ammos left text to white + remove the out of ammos text
				ResetAmmos:FireClient(self.player, self:getAmmoInWeapon())

				local cooldownLeft = self.nextFireTime - tick()
				if cooldownLeft > 0 then
					UpdateCooldown:FireClient(self.player, cooldownLeft, cooldownLeft / self:getConfigValue("ShotCooldown", 0.5))
				end

			else
				UpdateAmmoCount:FireClient(self.player, "∞")

				-- set the ammos left text to white + remove the out of ammos text
				ResetAmmos:FireClient(self.player)

				-- cancel the cooldown tween on the client
				CancelCooldown:FireClient(self.player)
			end
		end
	end
end
...
function BulletWeapon:onHit(hitInfo)
	...
	if not self:getConfigValue("ExplodeOnImpact", false) and hitPart and hitPart.Parent then
		--local humanoid = self.weaponsSystem.getHumanoid(hitPart)
		if hitPart.Name == "Zombie" and hitPart.Parent.Parent.Name == "Zombies" then
			hitInfo.h = hitPart.Parent
		else
			return
		end
		
		--Zombies:TakeDamage(hitInfo.h, self:getConfigValue("HitDamage", 10))
		
		-- headshot deals double damage
		if IsServer then
			--if hitPart.Name == "Head" then
				--DamageZombie:Fire(hitInfo.h, self:getConfigValue("HitDamage", 10), self.player)
			--else
			DamageZombie:Fire(hitInfo.h, self:getConfigValue("HitDamage", 10), self.player)
			--end
		end
	...
	if IsServer and self:getConfigValue("ExplodeOnImpact", false) then
		...
		explosion.Hit:Connect(function(explodedPart, hitDist)
			...
			if explodedPart.Parent and explodedPart.Name == "Zombie" then
				--local damageMultiplier = (1 - math.clamp((hitDist / blastRadius), 0, 1))
				--local damageToDeal = blastDamage * damageMultiplier

				if explodedPart.Parent.Parent.Name == "Zombies" then
					hitInfo.h = explodedPart.Parent
				else
					return
				end
...
function BulletWeapon:onFired(firingPlayer, fireInfo, fromNetwork)
	...
	local cooldownTime = self:getConfigValue("ShotCooldown", 0.5)
	...
	if IsServer then
		UpdateCooldown:FireClient(firingPlayer,  cooldownTime, 1)
		UpdateAmmoCount:FireClient(firingPlayer, self:getAmmoInWeapon() - 1)
		
		-- if the player has one of the starter packs, he might be running out of ammos without having the zone unlocked, thus he has infinite ammos until he unlocks the zone
		if self:getAmmoInWeapon() == 1 then
			if self.instance.Name == "SubMachineGun" and not Utilities:IsZoneUnlocked("Shop") then
				Weapons:ReloadWeapon(self.player, self.instance)
				
			elseif self.instance.Name == "Shotgun" and not Utilities:IsZoneUnlocked("Subway") then
				Weapons:ReloadWeapon(self.player, self.instance)
				
			elseif self.instance.Name == "Rifle" and not Utilities:IsZoneUnlocked("Garage") then
				Weapons:ReloadWeapon(self.player, self.instance)
				
			elseif self.instance.Name == "LaserGun" and not Utilities:IsZoneUnlocked("Forest") then
				Weapons:ReloadWeapon(self.player, self.instance)
			end	
		end
	end
