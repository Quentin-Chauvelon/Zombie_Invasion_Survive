...
--local HealthMachineRemoteFunction = ReplicatedStorage:WaitForChild("HealthMachine")
...
local weaponModifiersMachineGui : Frame = script.Parent:WaitForChild("Frame")
local levels : Frame = weaponModifiersMachineGui:WaitForChild("Levels")

local weaponModifiersMachineFillBar : Frame = workspace:WaitForChild("Machines"):WaitForChild("WeaponModifiersMachine"):WaitForChild("WeaponModifiersMachine"):WaitForChild("FillBar"):WaitForChild("SurfaceGui"):WaitForChild("BarToFill"):WaitForChild("FillingBar")

local lplr : Player = game.Players.LocalPlayer
local damageMultiplierLevel : IntValue = lplr:WaitForChild("Machines"):WaitForChild("DamageMultiplierMachine")
...
local function LevelClicked(level : number)
	...
	-- if the player hasn't already bought an higher level of damage multiplier
	if damageMultiplierLevel.Value < level then
		if MachineRemoteFunction:InvokeServer("DamageMultiplier", level) then
			...
			weaponModifiersMachineFillBar:TweenSize(UDim2.new(1,0, level / 10, 0), Enum.EasingDirection.In, Enum.EasingStyle.Linear, 3)
		end
	end
end
...
workspace.Machines.WeaponModifiersMachine.ProximityPrompt.ProximityPrompt.Triggered:Connect(function(plr)
	weaponModifiersMachineGui.Visible = true
	weaponModifiersMachineGui:TweenPosition(UDim2.new(0.5,0,0.5,0), Enum.EasingDirection.InOut, Enum.EasingStyle.Linear, 0.5)
	...
	if not firstTrigger then
		...
		close = weaponModifiersMachineGui.Close.MouseButton1Down:Connect(function()
			if plr.Character and plr.Character:FindFirstChild("Humanoid") then
			...
			weaponModifiersMachineGui:TweenPosition(UDim2.new(0.5,0,1.5,0), Enum.EasingDirection.InOut, Enum.EasingStyle.Linear, 0.5)
			wait(0.5)
			weaponModifiersMachineGui.Visible = false
		end)
	end
end)