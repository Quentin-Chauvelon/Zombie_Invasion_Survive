...
function BulletWeapon:onHit(hitInfo)
	...
	if hitPart and hitPart.Parent then
		...
		if IsServer and humanoid and humanoid.Parent.Parent.Name == "Zombies" then
			if hitPart.Name == "Head" then
				DamageZombie:Fire(hitInfo.h, self:getConfigValue("HitDamage", 10) * 2)
			else
				DamageZombie:Fire(hitInfo.h, self:getConfigValue("HitDamage", 10))
			end
		end
	...
	if IsServer and self:getConfigValue("ExplodeOnImpact", false) then
		...
		explosion.Hit:Connect(function(explodedPart, hitDist)
			...
			local humanoid = self.weaponsSystem.getHumanoid(explodedPart)
			if humanoid and explodedPart.Name == "UpperTorso" and humanoid.Parent.Parent.Name == "Zombies" then
				DamageZombie:Fire(humanoid, damageToDeal, self.player)
